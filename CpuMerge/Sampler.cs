...
namespace CpuMerge
{
    public class Sampler
    {
        ...
        private bool OutOfRange(Int2 location, Image image)
        {
            ...
            {
                case Image.Basic:
                    return location.X < basicPosition.X || location.Y < basicPosition.Y || location.X >= basicPosition.X + basicSize.X ||
                           location.Y >= basicPosition.Y + basicSize.Y;
                case Image.Sample:
                    return location.X < samplePosition.X || location.Y < samplePosition.Y || location.X >= samplePosition.X + sampleSize.X ||
                           location.Y >= samplePosition.Y + sampleSize.Y;
                case Image.Target:
                    return location.X < targetPosition.X || location.Y < targetPosition.Y || location.X >= targetPosition.X + targetSize.X ||
                           location.Y >= targetPosition.Y + targetSize.Y;
            }
            return true;
        }
        ...
        private Color GetPixel(Int2 location, Image image)
        {
            ...
            if (image == Image.Basic)
            {
                ...
                Float2 scaled = new Float2(relative.X * basic.Pixels.GetLength(0), relative.Y * basic.Pixels.GetLength(1));
                if (!antialiasing)
                    return basic.Pixels[(int) Math.Round(scaled.X), (int) Math.Round(scaled.Y)];
                Int2 inArray = new Int2((int) scaled.X, (int) scaled.Y);
                Float2 betweenArray = new Float2(scaled.X % 1, scaled.Y % 1);
                Color mean = new Color();
                mean = ColorUtilities.AddColor(mean, basic.Pixels[inArray.X, inArray.Y], (1 - betweenArray.X) * (1 - betweenArray.Y));
                mean = ColorUtilities.AddColor(mean, basic.Pixels[inArray.X + 1, inArray.Y], betweenArray.X * (1 - betweenArray.Y));
                mean = ColorUtilities.AddColor(mean, basic.Pixels[inArray.X + 1, inArray.Y + 1], betweenArray.X * betweenArray.Y);
                mean = ColorUtilities.AddColor(mean, basic.Pixels[inArray.X, inArray.Y + 1], (1 - betweenArray.X) * betweenArray.Y);
                return mean;
            }
            else
            {
                Float2 relative = new Float2((float)(location.X - samplePosition.X) / sampleSize.X, (float)(location.Y - samplePosition.Y) / sampleSize.Y);
                Float2 scaled = new Float2(relative.X * sample.Pixels.GetLength(0), relative.Y * sample.Pixels.GetLength(1));
                if (!antialiasing)
                    return sample.Pixels[(int)Math.Round(scaled.X), (int)Math.Round(scaled.Y)];
                Int2 inArray = new Int2((int)scaled.X, (int)scaled.Y);
                Float2 betweenArray = new Float2(scaled.X % 1, scaled.Y % 1);
                Color mean = new Color();
                mean = ColorUtilities.AddColor(mean, sample.Pixels[inArray.X, inArray.Y], (1 - betweenArray.X) * (1 - betweenArray.Y));
                mean = ColorUtilities.AddColor(mean, sample.Pixels[inArray.X + 1, inArray.Y], betweenArray.X * (1 - betweenArray.Y));
                mean = ColorUtilities.AddColor(mean, sample.Pixels[inArray.X + 1, inArray.Y + 1], betweenArray.X * betweenArray.Y);
                mean = ColorUtilities.AddColor(mean, sample.Pixels[inArray.X, inArray.Y + 1], (1 - betweenArray.X) * betweenArray.Y);
                return mean;
            }
        }
    }
